...
- 不扩展目录外的公共协议和服务入口；公共校验复用第 7 节的模板标记判定，豁免对比度、
  W9 固定双栏、大字号及相邻标签限制和展示单位检查，其它检查不变。模板内容问题另行修复。
  本地 HTTP 调试补丁、凭据、画廊和安装包不属于提交范围。
...
也不因被跳过的检查触发质量修复。
展示单位校验及 Compact 的 W9、大字号和相邻标签限制也复用此判定，记录对应跳过日志；
其它 hard/semantic、现有高度检查、单位后处理和运行时 IF 限制保持不变。
独立调用对比度校验器使用同一模板根判断，符合条件整卡跳过，否则所有内容正常检查。
`has_fusion_template_root()` 保留既有方法名以兼容调用，但不再检查融球背景。

回归覆盖两种尺寸的普通模板、`2x2` 融球模板、`2x4` 融球主题门禁、预览模板、精确标识、
非模板并列节点及其它校验继续生效。
