...
**2x2 双业务生成前置约束**：最终展示恰好两个业务对象时立即锁定 S4，root 的直接子组件只能是上下两个 `136×64vp` 内容背板且间距 `8vp`，禁止标题区、底部 `action_area`、Button、ActionUnit 和 TimelineUnit；每个保留动作只能绑定所属内容背板，即使其中一个业务是带 `EnterMeeting` 的会议也不得生成独立“加入会议”按钮。

**2x4 双业务生成前置约束**：必须先按业务对象实例合并字段，再统计数据块；同一耳机对象的名称、连接状态、耳机仓电量和充电状态固定算一个数据块，禁止拆成“耳机连接”和“耳机仓电量”两个数据块。最终展示恰好两个业务数据块时立即锁定 W9，root 必须是 Row，直接且只能包含左右两个 `134×126vp` 大内容背板，间距 `8vp`；禁止复用 2x2 S4，禁止 `root Stack -> content Column`，禁止两个 `276×59vp` 背板上下排列，禁止任何其它上下双业务布局。禁止卡级公共标题、跨业务公共内容行、卡级 `actions/action_area` 和背板外按钮。每个业务的标题、数据和至多一个按钮必须全部放在自己的背板内，action 数量不增加、拆分或重排业务数据块，也不得为了容纳 action 改走 W10。

**2x4 四数据生成前置约束**：最终展示恰好四个业务数据块时立即锁定 W8，无论用户是否提到标题都禁止卡级标题、CardHeader、header 及标题空槽；安全内容区必须完整用于 `2×2` 网格，固定为四个 `134×59vp` 小内容背板，横纵间距均为 `8vp`。root/全尺寸 grid 之外不得增加 content 包装或其它一级区域，不得为标题把背板压缩成 `145×52vp` 等尺寸。
...
- `assetCandidates` 只规定允许使用的素材范围，除 8.3 节小内容蒙版的 1-2 项信息规则外不要求使用；不得为了使用候选素材而新增内容区或图标槽。
...
- `2x2` 在选择骨架和分配空间前，先保留用户明确指定部分，再按允许的图标位置、数量上限和区域互斥确定图标。优先级仅用于取舍，除 8.3 节小内容蒙版的 1-2 项信息规则外不要求使用图标或创建图标槽，允许全卡纯文字。仅用户明确要求本身超过 2 个时允许超额，不再增加其他可选图标；没有合法素材时不得伪造。
...
- `actionSurface` 是按钮背景色，`actionInk` 是按钮文字色，必须成对显式写 `#AARRGGBB`，按第十二节固定配对；浅色按钮为内容色 20% 按钮背景和 100% 内容，融球为白色 20% 按钮背景和 90% 文字，图标由转换器使用白色 60%。按钮文字使用 14/400-500。
...
- `content_area` 必须为 `Row -> [timeline, meeting_texts]`，固定 `itemMargin:8`，不得缩小或省略；`timeline` 使用 TimelineUnit，`meeting_texts` 固定高 `48vp`、`itemMargin:4`，只包含会议标题、时间、地点 Text。2x2 root 左边距为 12vp 时，时间线从 x=12vp 开始，文字列固定从 x=28vp 开始。会议标题必须作为第一行 `14fp/700` 展示：优先绑定 TaskSpec 的 `events[0].title`，其次使用用户明确给出的会议名称，两者都没有时固定写“日程”；禁止删除标题或改由时间、地点、`eventCount` 替代。时间、地点使用 `12fp/400`。时间、地点不得包装为 Row，不得生成时钟、日历、位置等 Image；使用 TimelineUnit 时该会议内容区必须为零个 Image。
...
- `2x2` 单业务 S1/S2/S3 的 root 直接内容组默认不超过 3 个，优先采用“标题/上下文 + 主显示组 + 可选动作或支撑组”。该结构绝对不能用于 S4；S4 的 root 只能直接包含上下两个内容蒙版。弱 footer、额外状态条和第二数据域不因字号较小就免于计数；出现第 4 组时必须合并到已有组或删除优先级最低的一组。
...
- 小内容蒙版统一指 2x2 S4 的 `136×64vp` 分区和 2x4 W8/W10 的 `134×59vp` 分区，只允许两行内容：第一行主数据固定 `14fp/700`、内容色 100%，第二行辅助数据固定 `12fp/400`、内容色 60%，不再增加独立业务标题行；需要突出量化主值时，第一行必须以该数字及其单位为主，不在数字前添加业务标签。蒙版内 1 项数据时只输出第一行，2 项时分别放第一、二行；有合法候选素材时，1-2 项必须使用一个右侧图标，无合法素材时不得伪造或留空槽。3 项时不使用任何右侧视觉，第一行放主数据，第二行将两个辅助数据写成单个 Text 并以 ` | ` 分隔；超过 3 项时按“用户明确要求 > 主数据 > 状态 > 其他辅助”只保留前三项，其余删除。
...
当 `2x2` 单业务 S1/S2/S3 的任一候选无法通过文本或布局压力检查时，强制回退为以下最小骨架，不继续横向压缩；S4 不得使用此回退骨架：
...
- 槽位：主信息最多两行（名称/状态或主值/辅助文字）+ 两个显式动作；两个动作必须都有精确候选且由用户明确要求。纯数字主值最大 `24fp/700`，带单位的未拆分数值字符串最大 `18fp/700`，辅助文字固定 `12fp/400`；不得套用其它骨架的 `30fp/38fp` 大数字。S3 的设备名属于主内容，不使用 CardHeader，也不放名称旁的右上角辅助图标。
- 禁止：三按钮、双信息域、第三行信息、30fp 或 38fp 主值。
...
- region：只允许上下纵堆，固定为 `root -> [zone_top, zone_bottom]`，root `itemMargin:8`；两 zone 必须同时存在且均为 `136×64vp`、borderRadius 16、内容色 10% 背板，且 `64 + 8 + 64 = 136`，禁止只生成其中一个 zone。不得增加 title/header/footer/action_area，也禁止左右双业务布局。每个 zone 必须完整遵守 8.3 节小内容蒙版规则，文字在左、图标或环图在右。
...
- 槽位：每方最多 3 项数据并压成固定两行；1-2 项有合法素材时每区使用 1 个右侧图标，3 项时不使用图标。独立图标与环中心图标二选一，日期、地点、状态等辅助文字前后仍禁止单独配图标；两个分区使用图标后，按钮和标题不再配图标。
- 禁止：总标题、左右双业务、三方分区、两方结构不同构和独立动作区。

## 9.2 2x4 固定骨架（v0.4 W 骨架 · W1→W10）

以下横版区域均受276×126vp内容预算约束。单业务增加可选标题时必须从正文槽位扣除20vp及标题间距；例如W1双背板无标题时为59+8+59，有标题且间距2vp时改为48+8+48，单背板相应由126vp减为104vp。不能在满高区域上直接叠加标题。

2x4 每张卡必须且只能选择一个骨架，允许在声明范围内微调，不得跨骨架拼接 region。先统计数据块数量：1 个在 W1-W7 中按内容选择，2 个强制 W9，3 个强制 W10，4 个强制 W8。通用约束：独立卡级标题只允许用于单数据块骨架，有则必须使用 20vp 高的 CardHeader 作为全尺寸前景 Column 首项，无则不生成标题及空槽；W8/W9/W10 严禁卡级 title、CardHeader、公共标题行及标题空槽；分栏一律等分或固定宽（136/134/130），侧栏固定宽统一 130vp；线性进度条一律 `strokeWidth:8`；弹性沉底用 `layoutWeight:1` 空容器占位（`flexShrink` 只收缩不拉伸，禁止用于沉底）；禁止 `justifyContent:"spaceBetween"` 制造三段均分（会把中部元素推离预期位置）。

### `W1-progress-aux`（进度 + 辅助区域）

- 用于：倒计时、强提醒、单主指标进度 + 侧栏辅助（同构事项 / 说明背板 / 双入口）；承载 1 主数值 + 1 进度 + 辅助 0-2 项。
- region：`content Row -> [progress_zone 136, aux_zone 130]` gap 10。progress_zone：`count`（默认 30fp、最大 38fp 纯数字 + 12-16fp 单位）作为第一行 → `tag 12fp/400` 放在数值下方 → `layoutWeight:1` spacer → 沉底 `Progress linear 136×8`（strokeWidth 8）+ 端标行（12fp/400/内容色 60%，条下方两端对齐）。aux_zone 三变体：var-a 双份区域（两张 59 高背板 + gap 8，内容色 10% 底、圆角 12，各「标题 12fp/400 + 副文 12fp/400」）；var-b 整体大区域（单背板 126 全高、圆角 14，标题 400 顶部 + 正文 400 沉底、中部留白，无动作）；var-c 双按钮（两枚 130×36 胶囊纵排 gap 10，沉底与进度条底对齐）。
...
- region：`kicker 12fp/400` 顶 + `lower Column 103 justifyContent:end`：`event 76`（主内容标题最大 18fp/500 + 正文 12fp/400 两行）+ `date 12fp/400`（贴正文下方沉底）；kicker 与标题之间大留白（上空下实）。
...
- region：可选 `CardHeader 20vp` + `main Row 104`：`ringArea 134`（`Stack 92`：Progress ring strokeWidth 8 + 环心纯数字 20fp/700，未拆分数值字符串最大 18fp/700）+ `info 134`（说明标题 16fp/400 / 值 14fp/400 / 状态 12fp/400，居中）。环形 Progress 本身视为所属业务主内容第一项，除可选 CardHeader 外不得在环前增加说明或强调文字。有 CardHeader 时与 main 的间距使用 2vp，确保总高不超过 126vp。
- 槽位：一个环主指标；右侧至多三行说明。
...
- 用于：三并列同构指标（健康概览等）。
- region：可选 `CardHeader 20vp` + `metrics Row 84`：三列（82/86/82）+ 两条竖 `Divider 1×64` 居中；每列固定为第一行 `value_row`（纯数字 24fp/700 + 同行单位 12fp/400）和下方 `label 12fp/400`，三列对应行严格水平对齐，列内 justifyContent center；字段无法拆分数值与单位时，第一行使用最大 18fp 的完整数值字符串，不生成空单位占位。
- 槽位：恰好三个指标；每列一个数值。
...
- region：可选 `CardHeader 20vp` + 间隔2vp + `body 104`：`progressSlot 50`（主数值行 20fp/700 + label 12fp/400 + `Progress linear 276×8 strokeWidth 8`）+ 间隔8vp + `details Row 46`（两背板 134×2 gap 8、圆角 10、内容色 10% 底，各「标题 12fp/400 + 值 12fp/400」）。
- 槽位：一个进度主指标 + 两个详情项。
...
- region：无卡级标题；使用 `276×126vp` 的 `2×2` 网格，两行两列的横纵 itemMargin 均为 8，每个小内容蒙版固定 `134×59vp`、padding 12、borderRadius 16、内容色 10% 背景，并完整遵守 8.3 节小内容蒙版的两行数据、右侧图标和数据删减规则。仅在蒙版内有 1-2 项信息且需要比例视觉时允许使用右侧 `40×40vp、strokeWidth:4` 的环形 Progress 与环心 20×20vp 图标，此时蒙版上下 padding 改为 9/10vp，左右仍为 12vp，确保 40vp 环图完整容纳；3 项信息时不得保留环图，否则不得为填充版面强加环图。
...
- region：无卡级总标题；`root Row` 以 `width/height:matchParent` 填满 `300×150vp` 画布，padding 12、itemMargin 8，直接包含左右两个 `134×126vp` 同级背板。两背板均为 Column、padding 12、borderRadius 16、内容色 10% 背板，内部可用空间均为 `110×102vp`。这是 2x4 双业务唯一合法的一级布局，不得套用 2x2 S4 的上下两个 `136×64vp` 结构，也不得放大为上下两个 `276×59vp` 背板。
- 每侧按 2x2 的层级组织；需要突出量化主值时按「数字主值第一行 + 业务文字/辅助信息 + 可选底部按钮 110×36vp」排列，业务文字不得放在数字前；没有突出数字时仍可按「业务标题 12fp/400 + 主内容 + 可选底部按钮」排列。有按钮时主内容使用剩余高度，每侧至多保留两行最小充分信息。业务标题只是背板内分区标题，不使用 CardHeader。
- action：每侧至多一个属于该业务的可见 Button 或图文按钮 Row，事件只绑定该按钮；root、背板和非按钮内容均不得绑定 `onClick`。只有一侧有动作时，另一侧不生成按钮或空槽；不得生成卡级 action_area。先固定左右业务数据块再放 action，禁止为按钮新增数据块或改成 W10；动作必须放入语义所属背板，音乐动作不得放进天气背板，未被用户明确要求且无法归属当前业务的动作必须删除。
- title：所有标题必须先归属到左侧或右侧业务；没有突出数字时放入对应背板首项，有突出数字时放在该数字下方。地点、对象名或主题描述只要语义属于其中一个业务，就只能作为该业务标题；禁止放在两块背板上方形成卡级 title、CardHeader 或公共标题行。
...
- region：无卡级标题；`root Row` 以 `width/height:"matchParent"` 填满 `300×150vp` 画布，padding 12、itemMargin 8，直接包含左侧 `134×126vp` 大内容蒙版和右侧 `134×126vp` Column。右侧 Column 使用 itemMargin 8，上下各放一个 `134×59vp` 小内容蒙版。三个内容蒙版均使用 padding 12、borderRadius 16、内容色 10% 背景。
- 左侧大内容蒙版需要突出量化主值时按「数字主值第一行 + 业务文字/辅助信息 + 可选底部 Button 110×36vp」组织，业务文字不得放在数字前；没有突出数字时仍按「业务标签 12fp/400 + 主内容 + 可选底部 Button」组织。有动作时只绑定内部按钮，大内容蒙版本身不绑定，主内容最多两行最小充分信息。右侧两个分区完整遵守 8.3 节小内容蒙版的两行数据、右侧图标和数据删减规则。
...
- 融球或用户指定的场景背景只配简单骨架和极少背板；纯色 root 的内容蒙版使用内容色 10%，仅 W8/W9/W10 按固定骨架使用 4/2/3 块内容蒙版，其他骨架不复制多个迷你卡片。
...
- 需要突出量化主值时，数字及其紧邻单位必须位于所属业务主内容区第一行，其他需要突出的文字放在数字下方。CardHeader 和会议日期上下文不计入主内容行；2x2 倒计时是位置例外，目标名称/标题在顶部居中，数字主值组在其下方的卡片中部，有动作时动作区仍沉底，禁止把数字移到标题上方；环形 Progress 视为主内容第一项；S4/W8/W9/W10 及并列指标在各自分区内分别执行。只有用户核心问题对应的量化主值适用本规则，日期、时间、航班号、电话号码和普通编号不得仅因包含数字而自动升级为主数字。
...
- 未拆分的数字与单位字符串（如 `7小时30分`、`29°C`、`82%`）按文字处理，默认 16fp、最大 18fp，但作为量化主值时仍必须放在所属业务主内容区第一行；拆分后纯数字最大 38fp，单位使用 12-16fp。S3 双按钮骨架空间有限，纯数字主值另行限制为最大 24fp，禁止使用 30fp 或 38fp。
...
- 时长类主读数优先绑定纯数字 path，单位 `分`、`小时` 另放小号 Text；schema 只有 `durationText:"25分钟"` 这类带单位字符串时禁止放进 30 号主读数，默认使用 16fp、最大 18fp 的 Text。
...
- 数值和单位拆分后，`32fp` 纯数字与 12-16fp 单位组成的主值组至少分配 `88vp`，允许负号或三位数时至少 `104vp`；未拆分字符串仍不得超过 18fp。
...
- `2x4` 底部动作区只有一个卡级 Button 或图文按钮 Row 时，该按钮固定为 `x:12vp、y:102vp、width:276vp、height:36vp`，距卡片底部必须正好 `12vp`；按钮必须是满高前景 Column 的最后一项，上方真实内容区使用 `layoutWeight:1` 占据剩余空间。禁止生成 `136/130/134vp` 的半宽单按钮或浮动底边距。W9 背板内按钮是唯一例外，固定宽 `110vp` 并留在所属业务背板内。
...
颜色角色只使用以下三个固定术语，必须按组件类型判断，禁止互换颜色、透明度或作用组件，也不得引入其他称呼：

- **卡片背景**：只作用于 root，使用纯色 `backgroundColor` 或融球 `design`。
- **内容背板**：只作用于承载业务内容的 Row/Column，使用对应内容色 10%。
- **按钮背景**：只作用于 Button、ActionUnit 或专用图文按钮 Row，使用对应内容色 20%；融球下使用白色 20%。

## 背景选择与业务映射

1. 先检查用户明确的颜色或背景要求；没有指定时，再按下列融球条件选择，不能仅因示例使用融球就照搬。
2. 除 S2 会议时间线亚型固定使用黄色纯色外，融球必须同时满足：`2x2`、单一业务、最终实际展示的独立语义信息不超过 3 项、只有一个内容组、显式动作不超过 1 个。本次请求若未在提示词末尾追加“本次请求未启用融球能力”的运行时限制，即视为运行时允许融球；满足上述全部条件且业务有对应融球效果时必须使用融球。任一条件不满足都使用浅色纯色，不能先选融球再通过删减用户必须信息凑条件。列表、网格、双分区、多组独立指标、多条日程或重复事项均不是简单内容，不使用融球。
...
| 纯色 | 业务 | 卡片背景 | 内容 100% | 次要内容 60% | 按钮背景 20% | 内容背板 10% |
|---|---|---|---|---|---|---|
...
| 融球效果 | 业务 | root design |
|---|---|---|
| 日程冷色 | 用户明确要求融球的单个会议 | `fusion-ball-schedule-cool` |
...
融球卡片背景沿用转换器的五套固定球色，不手写球色、渐变或装饰圆球。所有融球前景统一使用白色：内容 `#FFFFFFFF`、次要内容 `#99FFFFFF`、按钮背景 `#33FFFFFF`、按钮文字 `#E6FFFFFF`、按钮内单色图标 `#99FFFFFF`、内容背板 `#1AFFFFFF`，不再使用带色相的近白前景。
...
- `surface`：所有内容背板、zone 背板、标签底板均使用内容色 10%，不再使用 5% 或其他背板透明度档位。背板内文字仍按本卡内容色和主次层级取色，不另建子色板。
...
- 浅色纯色卡片背景：2x2/2x4 的 Button、ActionUnit 和专用图文按钮 Row，按钮背景都固定为本卡内容色 20%，按钮文字及单色图标固定为内容色 100%；S4 即使带 `onClick` 仍是内容背板，保持内容色 10%，不套按钮配色。禁止内容色 100% 实心按钮背景配白色文字或白色图标，也不能换成不透明浅面、白底或另取更深的按钮文字色。
- 任何带 `onClick`、高度 `36vp`、圆角 `18-20vp` 的 Row 都必须按图文按钮处理，即使组件 id 或内容被命名为 card、zone、info、hint；必须使用按钮背景和按钮前景色，禁止使用内容色 100% 实心背景加白色文字。
- 融球卡片背景：按钮背景为白色 20%、文字为白色 90%、单色图标为白色 60%。内容背板白色 10% 与按钮背景白色 20% 是不同角色，严禁混用。
...
- 用户指定颜色、多色图标和真实状态色是上述固定取色的例外；不能借例外为普通背景、文字、按钮或内容背板自由配色。
...
5. 按第十二节先处理用户明确的配色要求，再判断融球条件，不满足时按业务选择固定浅色纯色；内容、按钮和背板跟随选定背景的固定配色。为素材确定职责，保留多色图标和真实状态色例外，不沿用旧渐变和自由取色规则。
