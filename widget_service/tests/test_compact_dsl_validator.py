...
@pytest.mark.parametrize("width,height", ((138, 134), (144, 136)))
def test_wide_dual_business_gate_uses_300_by_150_canvas(width: int, height: int) -> None:
    rows = [
        [
            "root", "Row",
            {"width": "matchParent", "height": "matchParent", "padding": 8, "itemMargin": 8},
            ["weather", "battery"],
        ],
        ["weather", "Column", {"width": width, "height": height, "padding": 12}, ["weather_body"]],
        ["battery", "Column", {"width": width, "height": height, "padding": 12}, ["battery_body"]],
        [
            "weather_body", "Column", {"layoutWeight": 1, "justifyContent": "center"},
            ["temperature"],
        ],
        ["battery_body", "Column", {"layoutWeight": 1, "justifyContent": "center"}, ["level"]],
    ...
    if width == 138:
        ...
        with pytest.raises(CompactDslValidationError, match="138x134"):
            validate_compact_dsl(compact_dsl, task_spec=task_spec, card_spec={"dataBindings": []})
...
def test_template_marker_skips_only_compact_design_rules(size, fusion, marker, caplog):
    ...
    if marker == "template_root":
        ...
        if marker == "detached":
            assert "Unreachable component(s): template_root" in message
        else:
